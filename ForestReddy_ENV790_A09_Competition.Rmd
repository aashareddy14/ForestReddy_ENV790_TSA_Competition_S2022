...
ggplot(demand, aes(x = date, y = demand)) + 
  geom_line() + 
  labs(title = "Time Series of Demand (2005 - 2010)", 
       x = "Year", 
       y = "Demand")
...
demand_train_ts <- ts(demand_train, start = 2001, frequency = 365)
demand_test_ts <- ts(demand_test, start = 2010, frequency = 365)
demand_ts <- ts(demand, start = 2005, frequency = 365)
```
...
```{r}
# observed data
observed <- demand_train[, "demand"] %>%
  pull()

# fit seasonal naive model - I'm not sure what h should be?
SNAIVE_seas <- snaive(demand_train_ts[,"demand"], h=31)
