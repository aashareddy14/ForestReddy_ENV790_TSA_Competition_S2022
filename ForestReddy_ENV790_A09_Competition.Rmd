...
First we will start by examining the demand data. We see that there is definitely a seasonal trend, and it looks like an increasing trend as well. We also see that the ACF plot has some scalloping pattern. 
...
demand_train_ts <- ts(demand_train$demand, start = 2005, frequency = 365)
demand_test_ts <- ts(demand_test$demand, start = 2010, frequency = 365)
demand_ts <- ts(demand$demand, start = 2005, frequency = 365)
```
...
decompose_demand <- decompose(demand_train_ts[, "demand"], "additive")
...
Confused on the below part - not sure how to get forecast. 
```{r}
# test data
test <- demand_test$demand

# fit seasonal naive model - I'm not sure what h should be? I think 365 because we are working
SNAIVE_seas <- snaive(demand_train_ts, h = 365)
checkresiduals(SNAIVE_seas)

# forecast for Jan 2010
SNAIVE_scores <- accuracy(SNAIVE_seas, test)
plot(SNAIVE_seas)

# now refit model using all data (2005 - 2010) - use demand_ts
SNAIVE_seas <- snaive(demand_ts, h = 365)

# load submission template and extract date column
submission_template <- read_excel("~/Desktop/Spring 2022/790_TimeSeries/ENV790_TimeSeriesAnalysis_Sp2022/Competition/Output/submission_template.xlsx", 
    sheet = "p90") %>% 
  select(date)

# forecast for Jan 2011 (this is what we submit to kaggle)
SNAIVE_forecast <- as.data.frame(SNAIVE_seas$model$future) 
SNAIVE_forecast <- cbind(submission_template, SNAIVE_forecast) %>%
  rename(load = x)

# next steps
  # only forecast for Jan 2011 - I am not sure if over forecasted by using h = 365
  # write.csv to save
```
...
```{r}
# fit SARIMA model - Not sure if I am doing this right.
SARIMA_autofit <- auto.arima(demand_test_ts, seasonal = TRUE)
checkresiduals(SARIMA_autofit)

SARIMA_for <- forecast(SARIMA_autofit, h = 365)

# forecast for Jan 2010
SARIMA_scores <- accuracy(SARIMA_for$mean, test)
plot(SARIMA_for) # this plot looks terrible!
