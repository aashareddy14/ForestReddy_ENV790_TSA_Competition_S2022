...
#demand_train_ts <- msts(demand_train$demand, seasonal.periods = c(12, 365.25))
...
We see that the residuals are normally distributed. However, there seems to be a pattern and significiant correlation in the ACF. We hope to improve on this in our next model.
...
```{r}
# I think we need to first fit on the train data, and then fit to test
# data afterwards - I'm going to keep this code chunk but do one below it that 
# I think is correct and then we can compare
...
- look at ACF and PACF plot and try to come up with a seasonal arima model on our own (SARAIMA_manual as opposed to SARIMA_autofit). (example from 07_Lesson, similar to this week's homework)
- ?? Not sure tbh
