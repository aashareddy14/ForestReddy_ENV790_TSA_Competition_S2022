...
jan_2010_temp <- df %>%
  ...
  select(avg_temp) 
jan_2010_temp <- as.vector(jan_2010_temp$avg_temp)
```
...
```{r}
# forecast for Jan 2010
SARIMAX_forecast_Jan_2010_temp <- forecast::forecast(SARIMAX_autofit_2009_temp, h = 31, 
                                                    xreg = jan_2010_temp)
SARIMAX_scores_Jan_2010_temp <- accuracy(SARIMAX_forecast_Jan_2010_temp$mean, test)
SARIMAX_scores_Jan_2010_temp
plot(SARIMAX_forecast_Jan_2010_temp)
```
...
# forecast for Jan 2011 (this is what we submit to kaggle)
SARIMAX_forecast_Jan_2011_temp <- forecast::forecast(SARIMAX_temp_autofit_2010,h = 31, xreg = jan_2010_temp) # ASSUMES THE SAME TEMP AS 2010! 
...
Next, we refit the model using all data 
```{r}
...
scores <- as.data.frame(
  rbind( SARIMA_scores_Jan_2010, SARIMA_scores_Jan_2010_temp, ARIMA_Four_for_2010_scores)
  )
row.names(scores) <- c( "SARIMA(1,2,2)","SARIMAX(3,0,2)_temp", "ARIMA + Fourier")
